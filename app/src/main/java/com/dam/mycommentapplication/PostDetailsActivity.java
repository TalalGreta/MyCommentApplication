...
//        myuid = FirebaseAuth.getInstance().getCurrentUser().getUid();
    ...
    private void loadComments() {
      ...
      DatabaseReference reference = FirebaseDatabase.getInstance().getReference("Posts").child(postId).child("Comments");
        //DatabaseReference reference = FirebaseDatabase.getInstance().getReference("Posts").child().child("Comments");
    ...
    private void loadUserInfo() {
        ...
        myref.orderByChild("uid").equalTo("1").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for (DataSnapshot dataSnapshot1 : dataSnapshot.getChildren()) {
